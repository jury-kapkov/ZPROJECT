...
namespace P.R.O.J.E.C.T.Z.geometry
{
    class HalfSphere : Primitive
    {
        ...
        public HalfSphere(Point3D basePoint, double radius, double height, int segmentsCount, Color color) : base(basePoint, height, color)
        {
            Radius = radius;
            SegmentCount = Math.Max(segmentsCount, 4);
            UpdatePoints();
        }
